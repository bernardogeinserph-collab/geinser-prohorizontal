...
import{Building2,Users,Briefcase,Shield,CheckSquare,Wrench,TrendingUp,LogOut,Plus,Edit2,Trash2,X,CheckCircle,AlertTriangle,ArrowLeft,UserPlus,Bell,Camera,Download,MessageSquare,Zap,HardHat,FileSearch,ChevronRight}from'lucide-react'
...
function Modal({title,onClose,children,wide}){return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}><div style={{background:'#fff',borderRadius:20,padding:24,width:'100%',maxWidth:wide?720:480,maxHeight:'92vh',overflowY:'auto'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}><h2 style={{margin:0,fontSize:18,fontWeight:800,color:GD}}>{title}</h2><button onClick={onClose} style={{background:'#f3f4f6',border:'none',borderRadius:'50%',width:32,height:32,cursor:'pointer'}}><X size={16}/></button></div>{children}</div></div>)}
...
const{data,loading,insert,update,remove}=useTable('pqrs',{copropiedad_id:copropiedad.id})
const[show,setShow]=useState(false);const[edit,setEdit]=useState(null);const[det,setDet]=useState(null)
const FD={numero:'',tipo:'Peticion',estado:'Recibida',nombre_solicitante:'',unidad:'',descripcion:'',fecha_recepcion:new Date().toISOString().slice(0,10),fecha_respuesta:'',fecha_cierre:'',responsable:'',respuesta:'',prioridad:'Normal',canal:'Escrito',notas:''}
...
async function save(){if(!form.descripcion){showT('Descripcion requerida','error');return}
const p={...form,copropiedad_id:copropiedad.id};if(edit){await update(edit.id,p);showT('PQR actualizada')}else{await insert(p);showT('PQR registrada')};setShow(false);setEdit(null);setForm(FD)}
...
<div style={{display:'flex',alignItems:'center',gap:10}}><h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>PQRs</h2><Bdg t={data.filter(p=>p.estado!=='Cerrada').length+' pendientes'} color="#dc2626"/></div>
<button onClick={()=>{setForm(FD);setEdit(null);setShow(true)}} style={{background:'#e53e3e',color:'#fff',border:'none',borderRadius:12,padding:'9px 18px',fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><Plus size={14}/>Nueva PQR</button>
</div>
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(290px,1fr))',gap:14}}>
{data.length===0&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:40,color:'#9ca3af'}}><MessageSquare size={40} color="#e5e7eb" style={{margin:'0 auto 12px',display:'block'}}/><p>Sin PQRs registradas</p></div>}
...
<div style={{display:'flex',gap:6,flexWrap:'wrap'}}><Bdg t={p.tipo} color="#7c3aed"/><Bdg t={p.estado} color={cE[p.estado?.replace(' ','')]||'#3b82f6'}/></div>
<div style={{display:'flex',gap:4}}>
<button onClick={e=>{e.stopPropagation();setForm({...FD,...p});setEdit(p);setShow(true)}} style={{background:'#f3f4f6',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer'}}><Edit2 size={11}/></button>
<button onClick={e=>{e.stopPropagation();if(window.confirm('Eliminar?'))remove(p.id)}} style={{background:'#fef2f2',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={11}/></button>
</div></div>
...
<div style={{fontSize:12,color:'#6b7280',marginBottom:6}}>{p.nombre_solicitante||'Anonimo'}{p.unidad?' · Unidad '+p.unidad:''}</div>
<div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#9ca3af'}}><span>Recibida: {fmt(p.fecha_recepcion)}</span>{p.fecha_cierre&&<span style={{color:'#059669'}}>Cerrada: {fmt(p.fecha_cierre)}</span>}</div>
</div>))}
</div>
...
<F label="Unidad / Apto"><input style={inp} value={form.unidad} onChange={e=>Fv('unidad',e.target.value)}/></F>
...
<div style={{gridColumn:'1/-1'}}><F label="Notas internas"><textarea style={{...inp,resize:'vertical',minHeight:45}} value={form.notas} onChange={e=>Fv('notas',e.target.value)}/></F></div>
</div>
...
{[['Descripcion',det.descripcion],['Solicitante',det.nombre_solicitante||'Anonimo'],['Unidad',det.unidad||'—'],['Estado',det.estado],['Prioridad',det.prioridad],['Canal',det.canal],['Responsable',det.responsable||'—'],['Recibida',fmt(det.fecha_recepcion)],['Respuesta',fmt(det.fecha_respuesta)],['Cierre',fmt(det.fecha_cierre)]].map(([l,v])=>(<div key={l} style={{background:'#f9fafb',borderRadius:8,padding:'9px 14px'}}><div style={{fontSize:10,color:'#9ca3af',fontWeight:700,marginBottom:1}}>{l.toUpperCase()}</div><div style={{fontSize:13,color:'#1f2937'}}>{v}</div></div>))}
{det.respuesta&&<div style={{background:'#e6f9f0',borderRadius:8,padding:'9px 14px'}}><div style={{fontSize:10,color:'#0d7a4a',fontWeight:700,marginBottom:1}}>RESPUESTA DADA</div><div style={{fontSize:13}}>{det.respuesta}</div></div>}
</div>
...
const{data,loading,insert,update,remove}=useTable('servicios_publicos',{copropiedad_id:copropiedad.id})
...
const FD={servicio:'Energia',periodo:'',fecha_factura:new Date().toISOString().slice(0,10),fecha_vencimiento:'',valor:0,consumo:0,unidad_consumo:'kWh',lectura_anterior:0,lectura_actual:0,estado:'Pendiente',observaciones:''}
const[form,setForm]=useState(FD);const Fv=(k,v)=>setForm(p=>({...p,[k]:v}))
const[toast,setToast]=useState(null);const showT=(m,t='success')=>{setToast({msg:m,type:t});setTimeout(()=>setToast(null),3000)}
async function save(){if(!form.servicio){showT('Servicio requerido','error');return}
const p={...form,copropiedad_id:copropiedad.id,valor:Number(form.valor)||0,consumo:Number(form.consumo)||0,lectura_anterior:Number(form.lectura_anterior)||0,lectura_actual:Number(form.lectura_actual)||0}
if(edit){await update(edit.id,p);showT('Actualizado')}else{await insert(p);showT('Registrado')};setShow(false);setEdit(null);setForm(FD)}
...
return(<div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
<h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Servicios Publicos</h2>
<button onClick={()=>{setForm(FD);setEdit(null);setShow(true)}} style={{background:'#f59e0b',color:'#fff',border:'none',borderRadius:12,padding:'9px 18px',fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><Plus size={14}/>Registrar factura</button>
</div>
{data.length===0&&<div style={{textAlign:'center',padding:48,color:'#9ca3af',background:'#fff',borderRadius:16,border:'1.5px solid #e5e7eb'}}><Zap size={40} color="#e5e7eb" style={{margin:'0 auto 12px',display:'block'}}/><p>Sin facturas registradas</p></div>}
...
<thead><tr style={{background:'#f9fafb'}}>{['Periodo','Factura','Vence','Consumo','Valor','Estado',''].map(h=><th key={h} style={{padding:'6px 10px',textAlign:'left',fontSize:10,color:'#9ca3af',fontWeight:700,whiteSpace:'nowrap'}}>{h}</th>)}</tr></thead>
...
<td style={{padding:'7px 10px',fontWeight:700}}>{f.periodo||'—'}</td>
<td style={{padding:'7px 10px'}}>{fmt(f.fecha_factura)}</td>
<td style={{padding:'7px 10px',color:daysTo(f.fecha_vencimiento)!==null&&daysTo(f.fecha_vencimiento)<=5?'#dc2626':'inherit'}}>{fmt(f.fecha_vencimiento)}</td>
<td style={{padding:'7px 10px'}}><div style={{display:'flex',alignItems:'center',gap:6}}><div style={{background:'#e5e7eb',borderRadius:99,height:5,width:55,flexShrink:0}}><div style={{background:esMax?'#dc2626':cS[serv]||'#3b82f6',borderRadius:99,height:5,width:pct+'%'}}/></div><span style={{fontWeight:700,color:esMax?'#dc2626':'inherit'}}>{f.consumo} {f.unidad_consumo}</span>{esMax&&<Bdg t="MAX" color="#dc2626"/>}</div></td>
<td style={{padding:'7px 10px',fontWeight:700}}>{fmtM(f.valor)}</td>
<td style={{padding:'7px 10px'}}><Bdg t={f.estado} color={f.estado==='Pagado'?'#059669':f.estado==='Vencido'?'#dc2626':'#d97706'}/></td>
<td style={{padding:'7px 10px'}}><div style={{display:'flex',gap:4}}><button onClick={()=>{setForm({...FD,...f,valor:String(f.valor),consumo:String(f.consumo),lectura_anterior:String(f.lectura_anterior),lectura_actual:String(f.lectura_actual)});setEdit(f);setShow(true)}} style={{background:'#f3f4f6',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer'}}><Edit2 size={11}/></button><button onClick={()=>{if(window.confirm('Eliminar?'))remove(f.id)}} style={{background:'#fef2f2',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={11}/></button></div></td>
</tr>)})}
</tbody></table></div></div>)})}
...
<F label="Estado"><select style={inp} value={form.estado} onChange={e=>Fv('estado',e.target.value)}><option>Pendiente</option><option>Pagado</option><option>Vencido</option><option>En disputa</option></select></F>
<div/>
...
const FD={nombre:'',descripcion:'',contratista:'',valor_contrato:0,fecha_inicio:'',fecha_fin_estimada:'',estado:'En ejecucion',porcentaje_avance:0,interventor:''}
...
async function save(){if(!form.nombre){showT('Nombre requerido','error');return}
const p={...form,copropiedad_id:copropiedad.id,valor_contrato:Number(form.valor_contrato)||0,porcentaje_avance:Number(form.porcentaje_avance)||0}
if(edit){await update(edit.id,p);showT('Actualizada')}else{await insert(p);showT('Obra creada')};setShow(false);setEdit(null);setForm(FD)}
async function saveR(){if(!formR.descripcion){showT('Descripcion requerida','error');return}
const{error}=await supabase.from('obra_reportes').insert({...formR,obra_id:obraAct.id,porcentaje_avance:Number(formR.porcentaje_avance)||0})
if(!error){showT('Reporte registrado');await loadR(obraAct);if(formR.porcentaje_avance)await update(obraAct.id,{porcentaje_avance:Number(formR.porcentaje_avance)});setShowR(false);setFormR(FDR)}
else showT('Error: '+error.message,'error')}
async function subirEv(e,rid){const file=e.target.files[0];if(!file)return;setSubiendo(true)
const path=`obras/${rid}/${Date.now()}.${file.name.split('.').pop()}`
await supabase.storage.from('geinser-fotos').upload(path,file,{upsert:true})
const{data:u}=supabase.storage.from('geinser-fotos').getPublicUrl(path)
await supabase.from('obra_reportes').update({evidencia:u.publicUrl}).eq('id',rid)
await loadR(obraAct);setSubiendo(false);showT('Foto subida')}
const cEst={'En ejecucion':'#3b82f6',Suspendida:'#d97706',Finalizada:'#059669',Cancelada:'#dc2626','En licitacion':'#8b5cf6'}
if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando...</div>
return(<div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
<h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Obras en Ejecucion</h2>
<button onClick={()=>{setForm(FD);setEdit(null);setShow(true)}} style={{background:'#f97316',color:'#fff',border:'none',borderRadius:12,padding:'9px 18px',fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><Plus size={14}/>Nueva obra</button>
</div>
{obraAct?(<div>
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12}}><div><div style={{fontWeight:900,fontSize:18,color:GD}}>{obraAct.nombre}</div><div style={{fontSize:12,color:'#6b7280'}}>{obraAct.contratista||'Sin contratista'}</div></div><Bdg t={obraAct.estado} color={cEst[obraAct.estado]||'#3b82f6'}/></div>
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:6}}>{[['Valor',fmtM(obraAct.valor_contrato)],['Inicio',fmt(obraAct.fecha_inicio)],['Fin estimado',fmt(obraAct.fecha_fin_estimada)]].map(([l,v])=><div key={l} style={{background:'#f9fafb',borderRadius:8,padding:'7px 10px'}}><div style={{fontSize:9,color:'#9ca3af'}}>{l}</div><div style={{fontSize:12,fontWeight:700}}>{v}</div></div>)}</div>
{obraAct.interventor&&<div style={{marginTop:8,fontSize:12,color:'#6b7280'}}>Interventor: <b>{obraAct.interventor}</b></div>}
</div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><h3 style={{margin:0,fontSize:16,fontWeight:800,color:GD}}>Reportes ({reportes.length})</h3><button onClick={()=>{setFormR(FDR);setShowR(true)}} style={{background:'#f97316',color:'#fff',border:'none',borderRadius:10,padding:'8px 14px',fontWeight:800,cursor:'pointer',fontSize:13,display:'flex',alignItems:'center',gap:5}}><Plus size={13}/>Nuevo reporte</button></div>
...
{rep.evidencia&&<img src={rep.evidencia} alt="ev" style={{width:'100%',maxHeight:160,objectFit:'cover',borderRadius:8,marginTop:6}}/>}
<label style={{display:'inline-flex',alignItems:'center',gap:4,marginTop:8,background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:7,padding:'5px 10px',cursor:'pointer',color:'#f97316',fontSize:11,fontWeight:700}}><Camera size={11}/>Foto<input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subirEv(e,rep.id)} disabled={subiendo}/></label>
</div>))}
</div>
{showR&&<Modal title="Nuevo reporte de obra" onClose={()=>setShowR(false)} wide>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
...
<div style={{gridColumn:'1/-1'}}><F label="Descripcion del avance" req><textarea style={{...inp,resize:'vertical',minHeight:65}} value={formR.descripcion} onChange={e=>setFormR(p=>({...p,descripcion:e.target.value}))}/></F></div>
...
<div style={{background:'#fff7ed',borderRadius:9,padding:7}}><HardHat size={18} color="#f97316"/></div>
<div style={{display:'flex',gap:5}}><Bdg t={o.estado} color={cEst[o.estado]||'#3b82f6'}/>
<button onClick={e=>{e.stopPropagation();setForm({...FD,...o,valor_contrato:String(o.valor_contrato),porcentaje_avance:String(o.porcentaje_avance)});setEdit(o);setShow(true)}} style={{background:'#f3f4f6',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer'}}><Edit2 size={11}/></button></div>
</div>
<div style={{fontWeight:900,fontSize:15,color:GD,marginBottom:4}}>{o.nombre}</div>
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
<div style={{gridColumn:'1/-1'}}><F label="Nombre de la obra" req><input style={inp} value={form.nombre} onChange={e=>setForm(p=>({...p,nombre:e.target.value}))}/></F></div>
...
<F label="Valor contrato ($)"><input style={inp} type="number" value={form.valor_contrato} onChange={e=>setForm(p=>({...p,valor_contrato:e.target.value}))}/></F>
...
<F label="Fecha fin estimada"><input style={inp} type="date" value={form.fecha_fin_estimada} onChange={e=>setForm(p=>({...p,fecha_fin_estimada:e.target.value}))}/></F>
<F label="Avance actual (%)"><input style={inp} type="number" min="0" max="100" value={form.porcentaje_avance} onChange={e=>setForm(p=>({...p,porcentaje_avance:e.target.value}))}/></F><div/>
...
<div style={{display:'flex',gap:10,marginTop:8}}><button onClick={()=>{setShow(false);setEdit(null)}} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:10,padding:11,cursor:'pointer',fontWeight:700}}>Cancelar</button><button onClick={save} style={{flex:2,background:'#f97316',color:'#fff',border:'none',borderRadius:10,padding:11,cursor:'pointer',fontWeight:800}}>{edit?'Guardar':'Crear obra'}</button></div>
</Modal>}
{toast&&<Toast {...toast}/>}
</div>)
}
function Cotizaciones({copropiedad}){
const{data,loading,insert,update,remove}=useTable('cotizaciones',{copropiedad_id:copropiedad.id})
const[show,setShow]=useState(false);const[edit,setEdit]=useState(null)
const FD={numero:'',descripcion:'',solicitante:'',tipo:'Reparacion',estado:'Solicitada',proveedor:'',valor:0,fecha_solicitud:new Date().toISOString().slice(0,10),fecha_recepcion:'',fecha_vencimiento:'',aprobada:false,observaciones:''}
const[form,setForm]=useState(FD);const Fv=(k,v)=>setForm(p=>({...p,[k]:v}))
const[toast,setToast]=useState(null);const showT=(m,t='success')=>{setToast({msg:m,type:t});setTimeout(()=>setToast(null),3000)}
async function save(){if(!form.descripcion){showT('Descripcion requerida','error');return}
const p={...form,copropiedad_id:copropiedad.id,valor:Number(form.valor)||0}
if(edit){await update(edit.id,p);showT('Actualizada')}else{await insert(p);showT('Cotizacion registrada')};setShow(false);setEdit(null);setForm(FD)}
const cEst={Solicitada:'#3b82f6',Recibida:'#8b5cf6',EnEvaluacion:'#d97706',Aprobada:'#059669',Rechazada:'#dc2626',Vencida:'#6b7280'}
if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando...</div>
return(<div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
<h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Cotizaciones</h2>
<button onClick={()=>{setForm(FD);setEdit(null);setShow(true)}} style={{background:'#8b5cf6',color:'#fff',border:'none',borderRadius:12,padding:'9px 18px',fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><Plus size={14}/>Nueva cotizacion</button>
</div>
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(290px,1fr))',gap:14}}>
{data.length===0&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:48,color:'#9ca3af',background:'#fff',borderRadius:16,border:'1.5px solid #e5e7eb'}}><FileSearch size={40} color="#e5e7eb" style={{margin:'0 auto 12px',display:'block'}}/><p>Sin cotizaciones</p></div>}
{data.map(ct=>(<div key={ct.id} style={{background:'#fff',border:`1.5px solid ${ct.aprobada?'#bbf7d0':'#e5e7eb'}`,borderRadius:14,padding:16}}>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
<div style={{display:'flex',gap:6,flexWrap:'wrap'}}><Bdg t={ct.tipo} color="#8b5cf6"/><Bdg t={ct.estado} color={cEst[ct.estado?.replace(' ','')]||'#3b82f6'}/>{ct.aprobada&&<Bdg t="✓ Aprobada" color="#059669" bg="#dcfce7"/>}</div>
<div style={{display:'flex',gap:4}}><button onClick={()=>{setForm({...FD,...ct,valor:String(ct.valor)});setEdit(ct);setShow(true)}} style={{background:'#f3f4f6',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer'}}><Edit2 size={11}/></button><button onClick={()=>{if(window.confirm('Eliminar?'))remove(ct.id)}} style={{background:'#fef2f2',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={11}/></button></div>
</div>
...
<div style={{fontSize:11,color:'#9ca3af'}}>Solicitud: {fmt(ct.fecha_solicitud)}{ct.fecha_recepcion&&' · Recibida: '+fmt(ct.fecha_recepcion)}</div>
...
{show&&<Modal title={edit?'Editar cotizacion':'Nueva cotizacion'} onClose={()=>{setShow(false);setEdit(null)}} wide>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
...
<F label="Solicitante (consejo/asamblea)"><input style={inp} value={form.solicitante} onChange={e=>Fv('solicitante',e.target.value)}/></F>
...
<div style={{gridColumn:'1/-1'}}><F label="Descripcion / Alcance" req><textarea style={{...inp,resize:'vertical',minHeight:60}} value={form.descripcion} onChange={e=>Fv('descripcion',e.target.value)}/></F></div>
...
function Contratos({copropiedad}){return<ModuloBase tabla="contratos" copropiedad={copropiedad} titulo="Contratos" color="#7c3aed" FD={{proveedor:'',servicio:'',valor:'',fecha_inicio:'',fecha_vencimiento:'',contacto:'',telefono:'',estado:'Activo',notas:''}} campos={[{key:'proveedor',label:'Proveedor'},{key:'servicio',label:'Servicio'},{key:'valor',label:'Valor ($)',type:'number'},{key:'estado',label:'Estado',type:'select',options:['Activo','Vencido','Cancelado','En renovacion']},{key:'fecha_inicio',label:'Inicio',type:'date'},{key:'fecha_vencimiento',label:'Vencimiento',type:'date'},{key:'contacto',label:'Contacto'},{key:'telefono',label:'Telefono'},{key:'notas',label:'Notas',type:'textarea',full:true}]} renderCard={(ct,onE,onD)=>{const d=daysTo(ct.fecha_vencimiento);return(<div key={ct.id} style={{background:'#fff',border:`1.5px solid ${d!==null&&d>=0&&d<=60?'#fca5a5':'#e5e7eb'}`,borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={ct.estado} color="#7c3aed"/>{d!==null&&d>=0&&d<=60&&<Bdg t={d+'d'} color="#dc2626"/>}</div><div style={{fontWeight:900,color:'#7c3aed'}}>{ct.proveedor}</div><div style={{fontSize:12,color:GD,fontWeight:700,marginBottom:6}}>{ct.servicio}</div><div style={{fontSize:11,color:'#6b7280',marginBottom:8}}>{fmtM(ct.valor)}/mes · Vence {fmt(ct.fecha_vencimiento)}</div><div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}}/>}
function Tareas({copropiedad}){const cP={Alta:'#dc2626',Media:'#d97706',Baja:'#059669'};return<ModuloBase tabla="tareas" copropiedad={copropiedad} titulo="Tareas" color={GB} FD={{titulo:'',descripcion:'',prioridad:'Media',estado:'Pendiente',progreso:0,fecha_limite:'',responsable:'',evidencia:''}} campos={[{key:'titulo',label:'Titulo',full:true},{key:'descripcion',label:'Descripcion',type:'textarea',full:true},{key:'prioridad',label:'Prioridad',type:'select',options:['Alta','Media','Baja']},{key:'estado',label:'Estado',type:'select',options:['Pendiente','En progreso','Completada','Cancelada']},{key:'fecha_limite',label:'Fecha limite',type:'date'},{key:'responsable',label:'Responsable'},{key:'progreso',label:'Avance (%)',type:'number'}]} renderCard={(t,onE,onD,subF,sub)=>(<div key={t.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={t.prioridad} color={cP[t.prioridad]||'#888'}/><Bdg t={t.estado} color={t.estado==='Completada'?'#059669':'#6b7280'}/></div><div style={{fontWeight:900,color:GD,marginBottom:4}}>{t.titulo}</div><div style={{marginBottom:8}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{fontSize:11,color:'#6b7280'}}>Avance</span><span style={{fontSize:11,fontWeight:700,color:GB}}>{t.progreso||0}%</span></div><div style={{background:'#e5e7eb',borderRadius:99,height:6}}><div style={{background:GB,borderRadius:99,height:6,width:(t.progreso||0)+'%'}}/></div></div>{t.evidencia&&<img src={t.evidencia} alt="foto" style={{width:'100%',height:100,objectFit:'cover',borderRadius:8,marginBottom:6}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#e8f4fd',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:GB,display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,t.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}/>}
function Mantenimiento({copropiedad}){return<ModuloBase tabla="mantenimientos" copropiedad={copropiedad} titulo="Mantenimiento" color="#059669" FD={{area:'',proveedor:'',periodicidad:'Mensual',ultima_fecha:'',proxima_fecha:'',costo:'',estado:'Programado',notas:''}} campos={[{key:'area',label:'Area / Sistema'},{key:'proveedor',label:'Proveedor'},{key:'periodicidad',label:'Periodicidad',type:'select',options:['Mensual','Bimestral','Trimestral','Semestral','Anual','Unico']},{key:'estado',label:'Estado',type:'select',options:['Programado','Ejecutado','Vencido','Cancelado']},{key:'ultima_fecha',label:'Ultima fecha',type:'date'},{key:'proxima_fecha',label:'Proxima fecha',type:'date'},{key:'costo',label:'Costo ($)',type:'number'},{key:'notas',label:'Notas',type:'textarea',full:true}]} renderCard={(m,onE,onD)=>{const d=daysTo(m.proxima_fecha);return(<div key={m.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={m.estado} color="#059669"/>{d!==null&&d>=0&&d<=7&&<Bdg t={d+'d'} color="#d97706"/>}</div><div style={{fontWeight:900,color:GD}}>{m.area}</div><div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>{m.proveedor||'Sin proveedor'} · {m.periodicidad}</div><div style={{fontSize:11,color:'#6b7280',marginBottom:8}}>Proxima: {fmt(m.proxima_fecha)} · {fmtM(m.costo)}</div><div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}}/>}
function Cartera({copropiedad}){return<ModuloBase tabla="cartera" copropiedad={copropiedad} titulo="Cartera" color="#dc2626" FD={{concepto:'',deudor:'',unidad:'',monto_deuda:'',monto_recuperado:'',fecha_mora:'',estado:'En mora',accion:'',notas:''}} campos={[{key:'concepto',label:'Concepto'},{key:'deudor',label:'Deudor'},{key:'unidad',label:'Unidad'},{key:'estado',label:'Estado',type:'select',options:['En mora','En acuerdo','Recuperado','Juridico','Castigado']},{key:'monto_deuda',label:'Deuda ($)',type:'number'},{key:'monto_recuperado',label:'Recuperado ($)',type:'number'},{key:'fecha_mora',label:'Fecha mora',type:'date'},{key:'accion',label:'Accion tomada'},{key:'notas',label:'Notas',type:'textarea',full:true}]} renderCard={(ct,onE,onD)=>(<div key={ct.id} style={{background:'#fff',border:'1.5px solid #fecaca',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={ct.estado} color="#dc2626"/><span style={{fontSize:11,fontWeight:700,color:'#dc2626'}}>{fmtM(ct.monto_deuda)}</span></div><div style={{fontWeight:900,color:GD}}>{ct.concepto}</div><div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>{ct.deudor||'—'}{ct.unidad?' · '+ct.unidad:''}</div>{ct.monto_recuperado>0&&<div style={{fontSize:11,color:'#059669',marginBottom:4}}>Recuperado: {fmtM(ct.monto_recuperado)}</div>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}/>}
function SGSST({copropiedad}){return<ModuloBase tabla="sgsst" copropiedad={copropiedad} titulo="SG-SST" color="#0891b2" FD={{actividad:'',fecha_programada:'',fecha_ejecutada:'',responsable:'',completada:false,evidencia:'',observaciones:''}} campos={[{key:'actividad',label:'Actividad',full:true},{key:'responsable',label:'Responsable'},{key:'fecha_programada',label:'Programada',type:'date'},{key:'fecha_ejecutada',label:'Ejecutada',type:'date'},{key:'completada',label:'Completada',type:'checkbox'},{key:'observaciones',label:'Observaciones',type:'textarea',full:true}]} renderCard={(a,onE,onD,subF,sub)=>(<div key={a.id} style={{background:'#fff',border:'1.5px solid #e0f2fe',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={a.completada?'Ejecutada':'Pendiente'} color={a.completada?'#059669':'#0369a1'}/></div><div style={{fontWeight:900,color:GD,marginBottom:4}}>{a.actividad}</div><div style={{fontSize:11,color:'#6b7280',marginBottom:4}}>Programada: {fmt(a.fecha_programada)}</div>{a.responsable&&<div style={{fontSize:11,color:'#6b7280',marginBottom:6}}>Responsable: {a.responsable}</div>}{a.evidencia&&<img src={a.evidencia} alt="ev" style={{width:'100%',height:90,objectFit:'cover',borderRadius:8,marginBottom:6}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#e0f2fe',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#0891b2',display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,a.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}/>}
function Alertas(){
...
if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando alertas...</div>
return(<div>
<div style={{display:'flex',alignItems:'center',gap:10,marginBottom:20}}><h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Panel de Alertas</h2><span style={{background:alerts.length>0?'#dc2626':'#059669',color:'#fff',borderRadius:99,padding:'2px 10px',fontSize:12,fontWeight:800}}>{alerts.length}</span></div>
{alerts.length===0&&<div style={{textAlign:'center',padding:60,color:'#9ca3af',background:'#fff',borderRadius:16,border:'1.5px solid #e5e7eb'}}><Bell size={48} color="#e5e7eb" style={{margin:'0 auto 16px',display:'block'}}/><p style={{fontSize:16}}>Sin alertas pendientes ✓</p></div>}
<div style={{display:'grid',gap:10}}>{alerts.map((a,i)=>(<div key={i} style={{background:a.color+'09',border:`1.5px solid ${a.color}30`,borderLeft:`4px solid ${a.color}`,borderRadius:12,padding:'13px 16px',display:'flex',alignItems:'flex-start',gap:12}}><span style={{fontSize:22,flexShrink:0}}>{a.icono}</span><div style={{flex:1}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:3}}><Bdg t={a.tipo} color={a.color}/>{a.u===3&&<Bdg t="URGENTE" color="#dc2626" bg="#dc2626"/>}</div><div style={{fontWeight:800,color:GD,fontSize:14}}>{a.msg}</div><div style={{fontSize:12,color:'#6b7280',marginTop:2}}>{a.sub}</div></div></div>))}</div>
</div>)
}
...
if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando...</div>
return(<div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}><h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Delegados ({delegados.length})</h2><button onClick={()=>setShow(true)} style={{background:GB,color:'#fff',border:'none',borderRadius:12,padding:'9px 18px',fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><UserPlus size={14}/>Nuevo delegado</button></div>
...
const subItems=[{key:'contratos',label:'Contratos',icon:Briefcase,color:'#7c3aed'},{key:'tareas',label:'Tareas',icon:CheckSquare,color:GB},{key:'mantenimiento',label:'Mantenimiento',icon:Wrench,color:'#059669'},{key:'cartera',label:'Cartera',icon:TrendingUp,color:'#dc2626'},{key:'sgsst',label:'SG-SST',icon:Shield,color:'#0891b2'},{key:'pqrs',label:'PQRs',icon:MessageSquare,color:'#e53e3e'},{key:'servicios',label:'Servicios Pub.',icon:Zap,color:'#f59e0b'},{key:'obras',label:'Obras',icon:HardHat,color:'#f97316'},{key:'cotizaciones',label:'Cotizaciones',icon:FileSearch,color:'#8b5cf6'}]
return(<div style={{display:'flex',minHeight:'100vh',fontFamily:"'Segoe UI',system-ui,sans-serif",background:'#f1f5f9'}}>
<div style={{width:220,background:GD,height:'100vh',position:'fixed',left:0,top:0,zIndex:200,display:'flex',flexDirection:'column'}}>
<div style={{padding:'16px 18px 12px',borderBottom:'1px solid rgba(255,255,255,0.1)'}}><svg width="110" height="28" viewBox="0 0 190 50"><rect width="44" height="44" rx="10" fill={GB} y="3"/><text x="22" y="33" textAnchor="middle" fill={GG} fontSize="24" fontWeight="900" fontFamily="Arial">G</text><text x="55" y="24" fill="white" fontSize="16" fontWeight="900" fontFamily="Arial" letterSpacing="1">GEINSER</text><text x="55" y="40" fill={GG} fontSize="9" fontWeight="700" fontFamily="Arial" letterSpacing="3">PROHORIZONTAL</text></svg></div>
...
{selCop&&subItems.map(({key,label,icon:Icon,color})=>(<button key={key} onClick={()=>setSubTab(key)} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'8px 18px 8px 22px',border:'none',background:subTab===key?'rgba(30,111,174,0.4)':'transparent',color:subTab===key?'#fff':'rgba(255,255,255,0.45)',cursor:'pointer',fontSize:11,fontWeight:subTab===key?700:400,textAlign:'left'}}><Icon size={13} color={subTab===key?'#fff':color}/>{label}</button>))}
</nav>
...
{selCop&&subTab==='tareas'&&<Tareas copropiedad={selCop}/>}
