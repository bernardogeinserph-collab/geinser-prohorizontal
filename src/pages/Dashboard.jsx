...
} function MiPerfil({perfil}){ const[pass1,setPass1]=useState('');const[pass2,setPass2]=useState('');const[saving,setSaving]=useState(false) const[toast,setToast]=useState(null);const showT=(m,t='success')=>{setToast({msg:m,type:t});setTimeout(()=>setToast(null),3500)} async function cambiarPassword(){   if(!pass1||pass1.length<6){showT('La contrasena debe tener al menos 6 caracteres','error');return}   if(pass1!==pass2){showT('Las contrasenas no coinciden','error');return}   setSaving(true)   const{error}=await supabase.auth.updateUser({password:pass1})   if(error){showT('Error: '+error.message,'error')}else{showT('Contrasena actualizada');setPass1('');setPass2('')}   setSaving(false) } return(<div style={{maxWidth:420}}> <h2 style={{margin:'0 0 20px',fontSize:20,fontWeight:900,color:GD}}>Mi Perfil</h2> <div style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:16,padding:20,marginBottom:20}}> <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:6}}> <div style={{width:48,height:48,background:'#e8f4fd',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center'}}><span style={{fontSize:20,fontWeight:900,color:GB}}>{perfil.nombre?perfil.nombre[0].toUpperCase():'?'}</span></div> <div><div style={{fontWeight:900,color:GD,fontSize:15}}>{perfil.nombre}</div><div style={{fontSize:12,color:'#9ca3af'}}>{perfil.email}</div></div> </div> <div style={{marginTop:8}}><Bdg t={(perfil.rol||'').toUpperCase()} color={GB}/></div> </div> <div style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:16,padding:20}}> <h3 style={{margin:'0 0 14px',fontSize:14,fontWeight:800,color:GD}}>Cambiar contrasena</h3> <F label="Nueva contrasena" req><input style={inp} type="password" value={pass1} onChange={e=>setPass1(e.target.value)} placeholder="Minimo 6 caracteres"/></F> <F label="Confirmar contrasena" req><input style={inp} type="password" value={pass2} onChange={e=>setPass2(e.target.value)} placeholder="Repite la contrasena"/></F> <button onClick={cambiarPassword} disabled={saving} style={{width:'100%',background:saving?'#9ca3af':GB,color:'#fff',border:'none',borderRadius:10,padding:11,cursor:saving?'not-allowed':'pointer',fontWeight:800,marginTop:6}}>{saving?'Guardando...':'Actualizar contrasena'}</button> </div> {toast&&<Toast msg={toast.msg} type={toast.type}/>} </div>) }
...
const[form,setForm]=useState(FD);const Fv=(k,v)=>setForm(p=>({...p,[k]:v}))
const[toast,setToast]=useState(null);const showT=(m,tp='success')=>{setToast({msg:m,type:tp});setTimeout(()=>setToast(null),3000)}
...
async function save(){if(!form.titulo){showT('Titulo requerido','error');return}
const payload={...form,copropiedad_id:copropiedad.id,progreso:Number(form.progreso)||0}
if(edit){const e2={fecha:new Date().toISOString(),tipo:'edicion',usuario:perfil?.nombre||'Usuario',nota:'Tarea editada'};await update(edit.id,{...payload,historial:[...(Array.isArray(edit.historial)?edit.historial:[]),e2]});showT('Actualizada')}
else{const e2={fecha:new Date().toISOString(),tipo:'creacion',usuario:perfil?.nombre||'Usuario',nota:'Tarea creada'};await insert({...payload,historial:[e2]});showT('Registrada')}
setShow(false);setEdit(null);setForm(FD)}
async function completar(tarea){const e2={fecha:new Date().toISOString(),tipo:'completada',usuario:perfil?.nombre||'Usuario',nota:'Marcada como completada'};const hist=[...(Array.isArray(tarea.historial)?tarea.historial:[]),e2];await update(tarea.id,{estado:'Completada',progreso:100,historial:hist});showT('Completada');refetch();if(det?.id===tarea.id)setDet(d=>({...d,estado:'Completada',progreso:100,historial:hist}))}
async function setAvance(tarea,pct){const e2={fecha:new Date().toISOString(),tipo:'avance',usuario:perfil?.nombre||'Usuario',nota:'Avance: '+pct+'%'};const hist=[...(Array.isArray(tarea.historial)?tarea.historial:[]),e2];await update(tarea.id,{progreso:pct,historial:hist});showT('Avance guardado');refetch();if(det?.id===tarea.id)setDet(d=>({...d,progreso:pct,historial:hist}));setAvRapido(null)}
async function addComentario(tarea){if(!comentNuevo.trim())return;const e2={fecha:new Date().toISOString(),tipo:'comentario',usuario:perfil?.nombre||'Usuario',nota:comentNuevo.trim()};const hist=[...(Array.isArray(tarea.historial)?tarea.historial:[]),e2];await update(tarea.id,{historial:hist,comentarios:((tarea.comentarios||'')+'
['+new Date().toLocaleDateString('es-CO')+'] '+(perfil?.nombre||'Usuario')+': '+comentNuevo.trim()).trim()});showT('Comentario guardado');setComentNuevo('');refetch();if(det?.id===tarea.id)setDet(d=>({...d,historial:hist}))}
async function addFoto(e,tarea){const file=e.target.files[0];if(!file)return;setSubiendo(true);const path='tareas/'+tarea.id+'/'+Date.now()+'.'+file.name.split('.').pop();const{error}=await supabase.storage.from('geinser-fotos').upload(path,file,{upsert:true});if(error){showT('Error foto','error');setSubiendo(false);return};const{data:u}=supabase.storage.from('geinser-fotos').getPublicUrl(path);const fa=[...(Array.isArray(tarea.fotos)?tarea.fotos:[]),u.publicUrl];const e2={fecha:new Date().toISOString(),tipo:'foto',usuario:perfil?.nombre||'Usuario',nota:'Foto agregada'};const hist=[...(Array.isArray(tarea.historial)?tarea.historial:[]),e2];await update(tarea.id,{fotos:fa,historial:hist});showT('Foto subida');setSubiendo(false);refetch();if(det?.id===tarea.id)setDet(d=>({...d,fotos:fa,historial:hist}))}
...
<div style={{display:'flex',alignItems:'center',gap:10}}><h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Tareas</h2><Bdg t={data.filter(t=>t.estado!=='Completada'&&t.estado!=='Cancelada').length+' activas'} color={GB}/></div>
...
{data.length===0&&<div style={{textAlign:'center',padding:60,color:'#9ca3af'}}><p>Sin tareas. Crea la primera tarea.</p></div>}
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:14}}>
{data.map(t=>{const dias=daysTo(t.fecha_limite);const venc=t.fecha_limite&&dias!==null&&dias<0&&t.estado!=='Completada';const prox=t.fecha_limite&&dias!==null&&dias>=0&&dias<=3&&t.estado!=='Completada';const fotos=Array.isArray(t.fotos)?t.fotos:[];return(<div key={t.id} style={{background:'#fff',border:'1.5px solid '+(venc?'#fca5a5':prox?'#fde68a':'#e5e7eb'),borderRadius:14,padding:16,cursor:'pointer'}} onClick={()=>setDet(t)}>
...
<label style={{background:'#e8f4fd',borderRadius:7,padding:'5px 8px',cursor:'pointer',color:GB,display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" capture="environment" style={{display:'none'}} onChange={e=>{e.stopPropagation();addFoto(e,t)}} disabled={subiendo}/></label>
<button onClick={e=>{e.stopPropagation();if(window.confirm('Eliminar?'))remove(t.id)}} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'5px 8px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button>
</div></div>)})}
</div>
...
{det.estado!=='Completada'&&(avRapido===det.id?<div style={{display:'flex',gap:5,alignItems:'center'}}>{[25,50,75,100].map(p=><button key={p} onClick={()=>setAvance(det,p)} style={{flex:1,background:GB,color:'#fff',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:12,fontWeight:700}}>{p}%</button>)<input type="number" min="0" max="100" defaultValue={det.progreso||0} onBlur={e=>setAvance(det,Number(e.target.value))} style={{...inp,width:55,textAlign:'center'}}/><button onClick={()=>setAvRapido(null)} style={{background:'#f3f4f6',border:'none',borderRadius:7,padding:'5px 8px',cursor:'pointer'}}>x</button></div>:<button onClick={()=>setAvRapido(det.id)} style={{width:'100%',background:'#f0f7ff',border:'1.5px solid #bfdbfe',borderRadius:8,padding:7,cursor:'pointer',color:GB,fontWeight:700,fontSize:12}}>Actualizar avance</button>)}</div>
...
<F label="Descripcion"><textarea style={{...inp,resize:'vertical',minHeight:65}} value={form.descripcion} onChange={e=>Fv('descripcion',e.target.value)} placeholder="Detalle..."/></F>
...
<F label="Fecha alerta"><input style={inp} type="date" value={form.fecha_alerta||''} onChange={e=>Fv('fecha_alerta',e.target.value)} title="Fecha en que recibirás alerta"/></F>
</div>
...
function Contratos({copropiedad}){return<ModuloBase tabla="contratos" copropiedad={copropiedad} titulo="Contratos" color="#7c3aed" FD={{proveedor:'',servicio:'',valor:'',fecha_inicio:'',fecha_vencimiento:'',contacto:'',telefono:'',estado:'Activo',notas:'',evidencia:''}} campos={[{key:'proveedor',label:'Proveedor'},{key:'servicio',label:'Servicio'},{key:'valor',label:'Valor ($)',type:'number'},{key:'estado',label:'Estado',type:'select',options:['Activo','Vencido','Cancelado','En renovacion']},{key:'fecha_inicio',label:'Inicio',type:'date'},{key:'fecha_vencimiento',label:'Vencimiento',type:'date'},{key:'contacto',label:'Contacto'},{key:'telefono',label:'Telefono'},{key:'notas',label:'Notas',type:'textarea',full:true}]} renderCard={(i,onE,onD,subF,sub)=>{const d=daysTo(i.fecha_vencimiento);return(<div key={i.id} style={{background:'#fff',border:`1.5px solid ${d!==null&&d>=0&&d<=60?'#fca5a5':'#e5e7eb'}`,borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={i.estado} color="#7c3aed"/>{d!==null&&d>=0&&d<=60&&<Bdg t={d+'d'} color="#dc2626"/>}</div><div style={{fontWeight:900,color:'#7c3aed'}}>{i.proveedor}</div><div style={{fontSize:12,color:GD,fontWeight:700,marginBottom:4}}>{i.servicio}</div><div style={{fontSize:11,color:'#6b7280',marginBottom:8}}>{fmtM(i.valor)}/mes · Vence {fmt(i.fecha_vencimiento)}</div>{i.evidencia&&<img src={i.evidencia} alt="foto" style={{width:'100%',height:100,objectFit:'cover',borderRadius:8,marginBottom:8}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#f5f3ff',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#7c3aed',display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,i.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}}/>}
function Tareas({copropiedad}){const cP={Alta:'#dc2626',Media:'#d97706',Baja:'#059669'};return<ModuloBase tabla="tareas" copropiedad={copropiedad} titulo="Tareas" color={GB} FD={{titulo:'',descripcion:'',prioridad:'Media',estado:'Pendiente',progreso:0,fecha_limite:'',responsable:'',evidencia:''}} campos={[{key:'titulo',label:'Titulo',full:true},{key:'descripcion',label:'Descripcion',type:'textarea',full:true},{key:'prioridad',label:'Prioridad',type:'select',options:['Alta','Media','Baja']},{key:'estado',label:'Estado',type:'select',options:['Pendiente','En progreso','Completada','Cancelada']},{key:'fecha_limite',label:'Fecha limite',type:'date'},{key:'responsable',label:'Responsable'},{key:'progreso',label:'Avance (%)',type:'number'}]} renderCard={(i,onE,onD,subF,sub)=>(<div key={i.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={i.prioridad} color={cP[i.prioridad]||'#888'}/><Bdg t={i.estado} color={i.estado==='Completada'?'#059669':'#6b7280'}/></div><div style={{fontWeight:900,color:GD,marginBottom:4}}>{i.titulo}</div><div style={{marginBottom:8}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{fontSize:11,color:'#6b7280'}}>Avance</span><span style={{fontSize:11,fontWeight:700,color:GB}}>{i.progreso||0}%</span></div><div style={{background:'#e5e7eb',borderRadius:99,height:6}}><div style={{background:GB,borderRadius:99,height:6,width:(i.progreso||0)+'%'}}/></div></div>{i.evidencia&&<img src={i.evidencia} alt="foto" style={{width:'100%',height:100,objectFit:'cover',borderRadius:8,marginBottom:8}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#e8f4fd',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:GB,display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,i.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}/>}
function Mantenimiento({copropiedad}){return<ModuloBase tabla="mantenimientos" copropiedad={copropiedad} titulo="Mantenimiento" color="#059669" FD={{area:'',proveedor:'',periodicidad:'Mensual',ultima_fecha:'',proxima_fecha:'',costo:'',estado:'Programado',notas:'',evidencia:''}} campos={[{key:'area',label:'Area / Sistema'},{key:'proveedor',label:'Proveedor'},{key:'periodicidad',label:'Periodicidad',type:'select',options:['Mensual','Bimestral','Trimestral','Semestral','Anual','Unico']},{key:'estado',label:'Estado',type:'select',options:['Programado','Ejecutado','Vencido','Cancelado']},{key:'ultima_fecha',label:'Ultima fecha',type:'date'},{key:'proxima_fecha',label:'Proxima fecha',type:'date'},{key:'costo',label:'Costo ($)',type:'number'},{key:'notas',label:'Notas',type:'textarea',full:true}]} renderCard={(i,onE,onD,subF,sub)=>{const d=daysTo(i.proxima_fecha);return(<div key={i.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={i.estado} color="#059669"/>{d!==null&&d>=0&&d<=7&&<Bdg t={d+'d'} color="#d97706"/>}</div><div style={{fontWeight:900,color:GD}}>{i.area}</div><div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>{i.proveedor||'Sin proveedor'} · {i.periodicidad}</div><div style={{fontSize:11,color:'#6b7280',marginBottom:8}}>Proxima: {fmt(i.proxima_fecha)} · {fmtM(i.costo)}</div>{i.evidencia&&<img src={i.evidencia} alt="foto" style={{width:'100%',height:100,objectFit:'cover',borderRadius:8,marginBottom:8}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#dcfce7',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#059669',display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,i.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}}/>}
function Cartera({copropiedad}){return<ModuloBase tabla="cartera" copropiedad={copropiedad} titulo="Cartera" color="#dc2626" FD={{concepto:'',deudor:'',unidad:'',monto_deuda:'',monto_recuperado:'',fecha_mora:'',estado:'En mora',accion:'',notas:'',evidencia:''}} campos={[{key:'concepto',label:'Concepto'},{key:'deudor',label:'Deudor'},{key:'unidad',label:'Unidad'},{key:'estado',label:'Estado',type:'select',options:['En mora','En acuerdo','Recuperado','Juridico','Castigado']},{key:'monto_deuda',label:'Deuda ($)',type:'number'},{key:'monto_recuperado',label:'Recuperado ($)',type:'number'},{key:'fecha_mora',label:'Fecha mora',type:'date'},{key:'accion',label:'Accion tomada'},{key:'notas',label:'Notas',type:'textarea',full:true}]} renderCard={(i,onE,onD,subF,sub)=>(<div key={i.id} style={{background:'#fff',border:'1.5px solid #fecaca',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={i.estado} color="#dc2626"/><span style={{fontSize:11,fontWeight:700,color:'#dc2626'}}>{fmtM(i.monto_deuda)}</span></div><div style={{fontWeight:900,color:GD}}>{i.concepto}</div><div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>{i.deudor||'—'}{i.unidad?' · '+i.unidad:''}</div>{i.monto_recuperado>0&&<div style={{fontSize:11,color:'#059669',marginBottom:4}}>Recuperado: {fmtM(i.monto_recuperado)}</div>}{i.evidencia&&<img src={i.evidencia} alt="foto" style={{width:'100%',height:100,objectFit:'cover',borderRadius:8,marginBottom:8}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#fef2f2',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626',display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,i.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}/>}
function SGSST({copropiedad}){return<ModuloBase tabla="sgsst" copropiedad={copropiedad} titulo="SG-SST" color="#0891b2" FD={{actividad:'',fecha_programada:'',fecha_ejecutada:'',responsable:'',completada:false,evidencia:'',observaciones:''}} campos={[{key:'actividad',label:'Actividad',full:true},{key:'responsable',label:'Responsable'},{key:'fecha_programada',label:'Programada',type:'date'},{key:'fecha_ejecutada',label:'Ejecutada',type:'date'},{key:'completada',label:'Completada',type:'checkbox'},{key:'observaciones',label:'Observaciones',type:'textarea',full:true}]} renderCard={(i,onE,onD,subF,sub)=>(<div key={i.id} style={{background:'#fff',border:'1.5px solid #e0f2fe',borderRadius:14,padding:16}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={i.completada?'Ejecutada':'Pendiente'} color={i.completada?'#059669':'#0369a1'}/></div><div style={{fontWeight:900,color:GD,marginBottom:4}}>{i.actividad}</div><div style={{fontSize:11,color:'#6b7280',marginBottom:4}}>Programada: {fmt(i.fecha_programada)}</div>{i.responsable&&<div style={{fontSize:11,color:'#6b7280',marginBottom:6}}>Responsable: {i.responsable}</div>}{i.evidencia&&<img src={i.evidencia} alt="ev" style={{width:'100%',height:90,objectFit:'cover',borderRadius:8,marginBottom:6}}/>}<div style={{display:'flex',gap:6}}><button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><label style={{background:'#e0f2fe',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#0891b2',display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,i.id)} disabled={sub}/></label><button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button></div></div>)}/>}
function Alertas(){
