...
const[rT,rC,rP,rM,rK,rQ,rS,rO,rRL]=await Promise.all([
...
supabase.from('representacion_legal').select('*').eq('copropiedad_id',cop.id)])
...
// Rep legal
if(repLegal.length){
y=seccion(y,'Representacion legal ante la alcaldia',[180,83,9])
doc.autoTable({startY:y,head:[['Representante','No. registro','Entidad','Inicio','Vencimiento','Estado']],
body:repLegal.map(x=>{const d=x.fecha_vencimiento?Math.ceil((new Date(x.fecha_vencimiento+'T00:00:00')-hoy2)/864e5):null;return[x.representante||'-',x.numero_registro||'-',x.entidad||'Alcaldia',x.fecha_inicio?new Date(x.fecha_inicio+'T00:00:00').toLocaleDateString('es-CO'):'-',x.fecha_vencimiento?new Date(x.fecha_vencimiento+'T00:00:00').toLocaleDateString('es-CO'):'-',d===null?'-':d<0?'VENCIDA':d<=90?'Vence en '+d+'d':'Vigente']}),
theme:'striped',headStyles:{fillColor:[180,83,9],fontSize:6.5},bodyStyles:{fontSize:6.5},margin:{left:14,right:14},
didParseCell:d=>{if(d.section==='body'&&d.column.index===5){const v=String(d.cell.raw);if(v==='VENCIDA')d.cell.styles.textColor=RO;else if(v.startsWith('Vence'))d.cell.styles.textColor=AM;else if(v==='Vigente')d.cell.styles.textColor=VE}}})
y=doc.lastAutoTable.finalY+6}
// 5. Tareas detalle
if(tP.length){
y=seccion(y,'Gestion de tareas del periodo',AB)
doc.autoTable({startY:y,head:[['Tarea','Estado','Prioridad','Avance','Fecha limite','Responsable','Costo']],
body:tP.map(t=>[t.titulo||'-',t.estado||'-',t.prioridad||'-',(t.progreso||0)+'%',t.fecha_limite?new Date(t.fecha_limite+'T00:00:00').toLocaleDateString('es-CO'):'-',t.responsable||'-',Number(t.costo)>0?fmtM(t.costo):'-']),
...
const costoEjec=comp.reduce((s,t)=>s+(Number(t.costo)||0),0)
const costoTotal=tP.reduce((s,t)=>s+(Number(t.costo)||0),0)
if(costoTotal>0){doc.setFontSize(8.5);doc.setTextColor(...AZ);doc.setFont(undefined,'bold')
doc.text('Costo de trabajos ejecutados (completados): '+fmtM(costoEjec)+'    |    Costo total registrado en el periodo: '+fmtM(costoTotal),14,y+3);doc.setFont(undefined,'normal');y+=8}
y+=3}
// 6. Mantenimientos
if(mantsP.length){
...
const FD={numero:'',tipo:'Peticion',estado:'Recibida',nombre_solicitante:'',unidad:'',descripcion:'',fecha_recepcion:new Date().toISOString().slice(0,10),fecha_respuesta:'',fecha_cierre:'',responsable:'',respuesta:'',prioridad:'Media',canal:'Escrito',notas:'',evidencia:''}
...
<div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}><div style={{display:'flex',gap:6,flexWrap:'wrap'}}><Bdg t={p.tipo} color="#7c3aed"/><Bdg t={p.prioridad||'Media'} color={p.prioridad==='Alta'?'#dc2626':p.prioridad==='Baja'?'#059669':'#d97706'}/><Bdg t={p.estado} color={cE[p.estado?.replace(' ','')]||'#3b82f6'}/></div><div style={{display:'flex',gap:4}}><button onClick={e=>{e.stopPropagation();setForm({...FD,...p});setEdit(p);setShow(true)}} style={{background:'#f3f4f6',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer'}}><Edit2 size={11}/></button><button onClick={e=>{e.stopPropagation();if(window.confirm('Eliminar?'))remove(p.id)}} style={{background:'#fef2f2',border:'none',borderRadius:6,padding:'3px 6px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={11}/></button></div></div>
...
<F label="Prioridad"><select style={inp} value={form.prioridad} onChange={e=>Fv('prioridad',e.target.value)}><option>Alta</option><option>Media</option><option>Baja</option></select></F>
...
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(280px,100%),1fr))',gap:14}}>{data.length===0&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:40,color:'#9ca3af'}}><p>Sin registros</p></div>}{data.map(item=>renderCard(item,()=>{setForm(Object.fromEntries(Object.keys(FD).map(k=>[k,item[k]??FD[k]])));setEdit(item);setShow(true)},()=>{if(window.confirm('Eliminar?'))remove(item.id)},subirFoto,subiendo,subirArchivo))}</div>
...
async function save(){
  ...
  const payload={...form,copropiedad_id:copropiedad.id,progreso:Number(form.progreso)||0,costo:Number(form.costo)||0,alerta_inactividad_dias:Number(form.alerta_inactividad_dias)||0,fecha_limite:form.fecha_limite||null,fecha_alerta:form.fecha_alerta||null}
  const res=edit
...
<div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}><Bdg t={det.prioridad} color={cP[det.prioridad]||'#888'}/><Bdg t={det.estado} color={cE[det.estado]||'#888'}/>{det.responsable&&<Bdg t={det.responsable} color="#6b7280"/>}{det.fecha_alerta&&<Bdg t={'Alerta: '+fmt(det.fecha_alerta)} color="#d97706"/>}{Number(det.costo)>0&&<Bdg t={'Costo: $'+Number(det.costo).toLocaleString('es-CO')} color="#0d2d4a"/>}</div>
...
<F label="Costo del trabajo ($)"><input style={inp} type="number" value={form.costo||''} onChange={e=>Fv('costo',e.target.value)} placeholder="0"/></F>
<F label="Alertar si no hay avance en (dias)"><input style={inp} type="number" value={form.alerta_inactividad_dias||''} onChange={e=>Fv('alerta_inactividad_dias',e.target.value)} placeholder="Ej: 5"/></F>
</div>
...
function Mantenimiento({copropiedad,perfil}){
const PERIODOS={'Mensual':1,'Bimestral':2,'Trimestral':3,'Semestral':6,'Anual':12,'Unico':0}
const FD={area:'',proveedor:'',periodicidad:'Mensual',ultima_fecha:'',proxima_fecha:'',costo:'',estado:'Programado',notas:'',evidencia:''}
const{data,loading,insert,update,remove,refetch}=useTable('mantenimientos',{copropiedad_id:copropiedad.id})
const[show,setShow]=useState(false);const[edit,setEdit]=useState(null)
const[form,setForm]=useState(FD);const Fv=(k,v)=>setForm(p=>({...p,[k]:v}))
const[toast,setToast]=useState(null);const showT=(m,tp='success')=>{setToast({msg:m,type:tp});setTimeout(()=>setToast(null),3500)}
const[subiendo,setSubiendo]=useState(false)
function sumarMeses(fechaStr,meses){const d=new Date(fechaStr+'T00:00:00');d.setMonth(d.getMonth()+meses);return d.toISOString().slice(0,10)}
function calcProxima(ultima,periodicidad){const m=PERIODOS[periodicidad];if(!ultima||!m)return '';return sumarMeses(ultima,m)}
async function save(){
if(!form.area){showT('El area/sistema es requerido','error');return}
const p={...form,copropiedad_id:copropiedad.id,costo:Number(form.costo)||0,ultima_fecha:form.ultima_fecha||null,proxima_fecha:form.proxima_fecha||(form.ultima_fecha?calcProxima(form.ultima_fecha,form.periodicidad):null)}
const res=edit?await update(edit.id,p):await insert(p)
if(res?.error){showT('Error: '+res.error.message,'error');return}
showT(edit?'Actualizado':'Mantenimiento registrado');setShow(false);setEdit(null);setForm(FD)}
async function marcarEjecutado(m){
const hoy=new Date().toISOString().slice(0,10)
const proxima=calcProxima(hoy,m.periodicidad)
const res=await update(m.id,{ultima_fecha:hoy,proxima_fecha:proxima||null,estado:m.periodicidad==='Unico'?'Ejecutado':'Programado'})
if(res?.error){showT('Error: '+res.error.message,'error');return}
showT('Ejecutado hoy'+(proxima?'. Proximo: '+fmt(proxima):''))}
async function subF(e,id){const file=e.target.files[0];if(!file)return;setSubiendo(true)
const path='mantenimientos/'+id+'/'+Date.now()+'.'+(file.name.split('.').pop()||'jpg')
const{error}=await supabase.storage.from('geinser-fotos').upload(path,file,{upsert:true})
if(error){showT('Error foto: '+error.message,'error');setSubiendo(false);return}
const{data:u}=supabase.storage.from('geinser-fotos').getPublicUrl(path)
const res=await update(id,{evidencia:u.publicUrl});setSubiendo(false)
if(res?.error){showT('Error: '+res.error.message,'error');return}
showT('Foto subida')}
const fmtM2=v=>'$'+(Number(v)||0).toLocaleString('es-CO')
if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando...</div>
const hoyS=new Date();hoyS.setHours(0,0,0,0)
const vencidos=data.filter(m=>m.proxima_fecha&&new Date(m.proxima_fecha+'T00:00:00')<hoyS&&m.estado!=='Cancelado'&&m.periodicidad!=='Unico').length
return(<div>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,flexWrap:'wrap',gap:8}}>
<div style={{display:'flex',alignItems:'center',gap:10}}><h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Mantenimiento</h2>{vencidos>0&&<Bdg t={vencidos+' vencidos'} color="#dc2626"/>}</div>
<button onClick={()=>{setForm(FD);setEdit(null);setShow(true)}} style={{background:'#059669',color:'#fff',border:'none',borderRadius:12,padding:'9px 18px',fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><Plus size={14}/>Nuevo</button>
</div>
{data.length===0&&<div style={{textAlign:'center',padding:50,color:'#9ca3af',background:'#fff',borderRadius:14,border:'1.5px dashed #e5e7eb'}}>Sin mantenimientos programados</div>}
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:14}}>
{data.map(m=>{
const dias=daysTo(m.proxima_fecha)
const venc=m.proxima_fecha&&dias!==null&&dias<0&&m.estado!=='Cancelado'
const prox=m.proxima_fecha&&dias!==null&&dias>=0&&dias<=7
const fProx=m.proxima_fecha?new Date(m.proxima_fecha+'T00:00:00'):null
return(<div key={m.id} style={{background:'#fff',border:'1.5px solid '+(venc?'#fca5a5':prox?'#fde68a':'#e5e7eb'),borderRadius:14,padding:16}}>
<div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={m.periodicidad||'Mensual'} color="#059669"/>{venc?<Bdg t={'VENCIDO hace '+Math.abs(dias)+'d'} color="#dc2626"/>:prox?<Bdg t={'Vence en '+dias+'d'} color="#d97706"/>:<Bdg t={m.estado||'Programado'} color={m.estado==='Ejecutado'?'#059669':'#6b7280'}/>}</div>
<div style={{fontWeight:900,color:GD,marginBottom:2}}>{m.area}</div>
{m.proveedor&&<div style={{fontSize:12,color:'#6b7280',marginBottom:8}}>{m.proveedor}</div>}
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginBottom:8,fontSize:11}}>
<div style={{background:'#f9fafb',borderRadius:8,padding:'6px 8px'}}><div style={{color:'#9ca3af',fontSize:9,fontWeight:700}}>ULTIMO</div><div style={{color:GD,fontWeight:700}}>{m.ultima_fecha?fmt(m.ultima_fecha):'—'}</div></div>
<div style={{background:venc?'#fef2f2':'#f0f7ff',borderRadius:8,padding:'6px 8px'}}><div style={{color:'#9ca3af',fontSize:9,fontWeight:700}}>PROXIMO</div><div style={{color:venc?'#dc2626':GB,fontWeight:800}}>{fProx?fProx.toLocaleDateString('es-CO',{day:'2-digit',month:'long'}):'—'}</div></div>
</div>
{venc&&<div style={{background:'#fef2f2',borderRadius:8,padding:'6px 10px',fontSize:11,color:'#dc2626',fontWeight:700,marginBottom:8}}>⚠ Se paso el periodo sin ejecutarlo</div>}
{Number(m.costo)>0&&<div style={{fontSize:11,color:'#6b7280',marginBottom:8}}>Costo: <b>{fmtM2(m.costo)}</b></div>}
{m.evidencia&&<img src={m.evidencia} alt="" style={{width:'100%',height:80,objectFit:'cover',borderRadius:8,marginBottom:8}}/>}
<div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
{m.periodicidad!=='Unico'||m.estado!=='Ejecutado'?<button onClick={()=>marcarEjecutado(m)} style={{background:'#f0fdf4',border:'none',borderRadius:7,padding:'5px 10px',cursor:'pointer',color:'#059669',fontSize:11,fontWeight:800}}>✓ Ejecutado hoy</button>:null}
<button onClick={()=>{setForm(Object.fromEntries(Object.keys(FD).map(k=>[k,m[k]??FD[k]])));setEdit(m);setShow(true)}} style={{background:'#f3f4f6',border:'none',borderRadius:7,padding:'5px 10px',cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button>
<label style={{background:'#e8f4fd',borderRadius:7,padding:'5px 8px',cursor:'pointer',color:GB,display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,m.id)} disabled={subiendo}/></label>
<button onClick={()=>{if(window.confirm('Eliminar?'))remove(m.id)}} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'5px 8px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button>
</div></div>)})}
</div>
{show&&<Modal title={edit?'Editar mantenimiento':'Nuevo mantenimiento'} onClose={()=>{setShow(false);setEdit(null);setForm(FD)}}>
<F label="Area / Sistema" req><input style={inp} value={form.area} onChange={e=>Fv('area',e.target.value)} placeholder="Ej: Recarga extintores, Ascensor, Motobomba..."/></F>
<F label="Proveedor"><input style={inp} value={form.proveedor} onChange={e=>Fv('proveedor',e.target.value)}/></F>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
<F label="Cada cuanto se ejecuta"><select style={inp} value={form.periodicidad} onChange={e=>{Fv('periodicidad',e.target.value);if(form.ultima_fecha)Fv('proxima_fecha',calcProxima(form.ultima_fecha,e.target.value))}}>{Object.keys(PERIODOS).map(o=><option key={o}>{o}</option>)}</select></F>
<F label="Estado"><select style={inp} value={form.estado} onChange={e=>Fv('estado',e.target.value)}>{['Programado','Ejecutado','Cancelado'].map(o=><option key={o}>{o}</option>)}</select></F>
<F label="Ultimo realizado"><input style={inp} type="date" value={form.ultima_fecha||''} onChange={e=>{Fv('ultima_fecha',e.target.value);Fv('proxima_fecha',calcProxima(e.target.value,form.periodicidad))}}/></F>
<F label="Proximo (auto)"><input style={inp} type="date" value={form.proxima_fecha||''} onChange={e=>Fv('proxima_fecha',e.target.value)}/></F>
</div>
<F label="Costo ($)"><input style={inp} type="number" value={form.costo} onChange={e=>Fv('costo',e.target.value)}/></F>
<F label="Notas"><textarea style={{...inp,resize:'vertical',minHeight:55}} value={form.notas||''} onChange={e=>Fv('notas',e.target.value)}/></F>
<button onClick={save} style={{width:'100%',background:'#059669',color:'#fff',border:'none',borderRadius:10,padding:11,fontWeight:800,cursor:'pointer',marginTop:4}}>{edit?'Guardar cambios':'Registrar'}</button>
</Modal>}
{toast&&<Toast msg={toast.msg} type={toast.type}/>}
</div>)
}
...
function RepresentacionLegal({copropiedad}){
return<ModuloBase tabla="representacion_legal" copropiedad={copropiedad} titulo="Representacion Legal" color="#b45309" FD={{representante:'',numero_registro:'',entidad:'Alcaldia',fecha_inicio:'',fecha_vencimiento:'',observaciones:'',evidencia:''}} campos={[{key:'representante',label:'Representante legal',full:true},{key:'numero_registro',label:'No. registro / radicado'},{key:'entidad',label:'Entidad (Alcaldia)'},{key:'fecha_inicio',label:'Inicio de vigencia',type:'date'},{key:'fecha_vencimiento',label:'Vencimiento del registro',type:'date'},{key:'observaciones',label:'Observaciones',type:'textarea',full:true}]} renderCard={(i,onE,onD,subF,sub)=>{const d=daysTo(i.fecha_vencimiento);const venc=d!==null&&d<0;const prox=d!==null&&d>=0&&d<=90;return(<div key={i.id} style={{background:'#fff',border:'1.5px solid '+(venc?'#fca5a5':prox?'#fde68a':'#e5e7eb'),borderRadius:14,padding:16}}>
<div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><Bdg t={i.entidad||'Alcaldia'} color="#b45309"/>{venc?<Bdg t="VENCIDA" color="#dc2626"/>:prox?<Bdg t={'Vence en '+d+'d'} color="#d97706"/>:i.fecha_vencimiento?<Bdg t="Vigente" color="#059669"/>:null}</div>
<div style={{fontWeight:900,color:GD,marginBottom:2}}>{i.representante||'Sin representante'}</div>
<div style={{fontSize:11,color:'#9ca3af',marginBottom:8}}>Registro No. {i.numero_registro||'-'}</div>
<div style={{background:'#f9fafb',borderRadius:8,padding:'6px 8px',marginBottom:8,fontSize:11}}><div style={{color:'#9ca3af',fontSize:9,fontWeight:700}}>VIGENCIA DEL REGISTRO</div><div style={{color:GD,fontWeight:700}}>{i.fecha_inicio?fmt(i.fecha_inicio):'-'} hasta {i.fecha_vencimiento?fmt(i.fecha_vencimiento):'-'}</div></div>
{i.observaciones&&<div style={{fontSize:11,color:'#6b7280',marginBottom:8}}>{i.observaciones}</div>}
{i.evidencia&&<img src={i.evidencia} alt="doc" style={{width:'100%',height:80,objectFit:'cover',borderRadius:8,marginBottom:8}}/>}
...
{i.archivo_pdf&&<a href={i.archivo_pdf} target="_blank" rel="noreferrer" style={{display:'block',background:'#fef9ec',border:'1.5px solid #f0c419',borderRadius:8,padding:'7px 10px',fontSize:12,fontWeight:800,color:'#92700c',textDecoration:'none',marginBottom:8}}>📄 Ver poliza (PDF)</a>}
<div style={{display:'flex',gap:6}}>
<button onClick={onE} style={{flex:1,background:'#f3f4f6',border:'none',borderRadius:7,padding:6,cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button>
<label style={{background:'#e8f4fd',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:GB,display:'flex',alignItems:'center'}}><Camera size={12}/><input type="file" accept="image/*" style={{display:'none'}} onChange={e=>subF(e,i.id)} disabled={sub}/></label>
<label style={{background:'#fef9ec',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#92700c',display:'flex',alignItems:'center',fontSize:11,fontWeight:800}} title="Subir PDF de la poliza">📎PDF<input type="file" accept="application/pdf" style={{display:'none'}} onChange={e=>upG(e,i.id,'archivo_pdf')} disabled={sub}/></label>
<button onClick={onD} style={{background:'#fef2f2',border:'none',borderRadius:7,padding:'6px 9px',cursor:'pointer',color:'#dc2626'}}><Trash2 size={12}/></button>
</div></div>)}}/>}

function Alertas(){
const[alerts,setAlerts]=useState([]);const[loading,setLoading]=useState(true)
useEffect(()=>{Promise.all([supabase.from('contratos').select('*,copropiedades(nombre)'),supabase.from('tareas').select('*,copropiedades(nombre)'),supabase.from('mantenimientos').select('*,copropiedades(nombre)'),supabase.from('cartera').select('*,copropiedades(nombre)'),supabase.from('pqrs').select('*,copropiedades(nombre)'),supabase.from('polizas').select('*,copropiedades(nombre)'),supabase.from('representacion_legal').select('*,copropiedades(nombre)')]).then(([c,t,m,k,p,pz,rl])=>{const a=[]
;(c.data||[]).forEach(x=>{const d=daysTo(x.fecha_vencimiento);if(d!==null&&d>=0&&d<=90)a.push({tipo:'Contrato',icono:'📄',color:'#dc2626',msg:x.proveedor+' — '+x.servicio,sub:'Vence en '+d+'d ('+fmt(x.fecha_vencimiento)+') · '+(x.copropiedades?.nombre||''),u:d<=15?3:d<=45?2:1})
if(x.fecha_vencimiento&&Number(x.dias_preaviso)>0&&x.estado==='Activo'){const fN=new Date(x.fecha_vencimiento+'T00:00:00');fN.setDate(fN.getDate()-Number(x.dias_preaviso));const dN=Math.ceil((fN-new Date().setHours(0,0,0,0))/864e5);if(dN>=-5&&dN<=20)a.push({tipo:'Notificar terminacion',icono:'📨',color:'#7c3aed',msg:x.proveedor+' — '+x.servicio,sub:(dN>=0?'Debes notificar la terminacion antes del '+fN.toLocaleDateString('es-CO',{day:'2-digit',month:'long'}):'¡Plazo de notificacion vencido hace '+Math.abs(dN)+'d!')+' · '+(x.copropiedades?.nombre||''),u:dN<=5?3:2})}})
;(rl?.data||[]).forEach(x=>{const d=daysTo(x.fecha_vencimiento);if(d!==null&&d>=0&&d<=90)a.push({tipo:'Rep. Legal',icono:'⚖️',color:'#b45309',msg:'Representacion legal — '+(x.representante||''),sub:'Vence en '+d+'d · '+(x.copropiedades?.nombre||''),u:d<=15?3:d<=45?2:1})})
;(pz?.data||[]).forEach(x=>{const d=daysTo(x.fin_vigencia);if(d!==null&&d>=0&&d<=40)a.push({tipo:'Poliza',icono:'🛡️',color:'#0891b2',msg:'Poliza '+(x.aseguradora||'')+' '+(x.numero_poliza||''),sub:'Vence el '+new Date(x.fin_vigencia+'T00:00:00').toLocaleDateString('es-CO',{day:'2-digit',month:'long'})+' ('+d+'d). Contacta al asesor para evitar que se cancele · '+(x.copropiedades?.nombre||''),u:d<=10?3:2})
if(d!==null&&d<0)a.push({tipo:'Poliza VENCIDA',icono:'🛡️',color:'#dc2626',msg:'Poliza '+(x.aseguradora||'')+' '+(x.numero_poliza||''),sub:'Vencida hace '+Math.abs(d)+'d. Contacta al asesor URGENTE · '+(x.copropiedades?.nombre||''),u:3})})
;(t.data||[]).forEach(x=>{const d=daysTo(x.fecha_limite);const activa=x.estado!=='Completada'&&x.estado!=='Cancelada'
if(d!==null&&d<0&&activa)a.push({tipo:'Tarea vencida',icono:'⚠️',color:'#b91c1c',msg:x.titulo,sub:'Vencio hace '+Math.abs(d)+'d · '+(x.copropiedades?.nombre||''),u:3})
if(d!==null&&d>=0&&d<=7&&activa)a.push({tipo:'Tarea por vencer',icono:'⏰',color:'#d97706',msg:x.titulo,sub:'Vence el '+new Date(x.fecha_limite+'T00:00:00').toLocaleDateString('es-CO',{day:'2-digit',month:'long'})+' ('+d+'d) · '+(x.copropiedades?.nombre||''),u:d<=2?3:2})
if(activa&&Number(x.alerta_inactividad_dias)>0){const hist=Array.isArray(x.historial)?x.historial:[];const ultima=hist.length?new Date(hist[hist.length-1].fecha):new Date(x.created_at);const diasSin=Math.floor((Date.now()-ultima.getTime())/864e5);if(diasSin>=Number(x.alerta_inactividad_dias))a.push({tipo:'Tarea sin actividad',icono:'🔕',color:'#7c3aed',msg:x.titulo,sub:'Sin actividad hace '+diasSin+' dias (limite: '+x.alerta_inactividad_dias+'d) · '+(x.copropiedades?.nombre||''),u:2})}})
;(m.data||[]).forEach(x=>{if(x.estado==='Cancelado')return;const d=daysTo(x.proxima_fecha);if(d!==null&&d>=0&&d<=7)a.push({tipo:'Mantenimiento',icono:'🔧',color:'#d97706',msg:x.area+' — '+(x.proveedor||''),sub:'Se vence el '+new Date(x.proxima_fecha+'T00:00:00').toLocaleDateString('es-CO',{day:'2-digit',month:'long'})+' ('+d+'d) · '+(x.copropiedades?.nombre||''),u:d<=2?3:2})
if(d!==null&&d<0)a.push({tipo:'Mantenimiento VENCIDO',icono:'🔧',color:'#dc2626',msg:x.area+' — '+(x.proveedor||''),sub:'Se paso el periodo sin realizarlo (hace '+Math.abs(d)+'d) · '+(x.copropiedades?.nombre||''),u:3})})
...
;(p.data||[]).filter(x=>x.estado!=='Cerrada').forEach(x=>a.push({tipo:'PQR '+x.tipo,icono:'📋',color:'#3b82f6',msg:(x.descripcion||'').slice(0,60),sub:(x.nombre_solicitante||'Anonimo')+' · '+(x.copropiedades?.nombre||''),u:x.prioridad==='Alta'?3:x.prioridad==='Media'?2:1}))
...
const resp=await fetch('https://xtqwbyfolbmiidiurmch.supabase.co/functions/v1/crear-delegado',{method:'POST',headers:{'Authorization':'Bearer '+session.access_token,'Content-Type':'application/json'},body:JSON.stringify({email:form.email.trim(),password:form.password,nombre:form.nombre,telefono:form.telefono||'',copropiedad_id:form.copropiedad_id||null})})
const data=await resp.json();if(!resp.ok||data.error)throw new Error(data.error||'Error creando delegado')
showT('Delegado creado. Se enviaron las credenciales a su correo.');setShow(false);setForm({nombre:'',email:'',password:'',telefono:'',copropiedad_id:''});refetch()}catch(e){showT('Error: '+e.message,'error')};setSaving(false)}
const[reenviando,setReenviando]=useState(null)
const[editDel,setEditDel]=useState(null);const[formE,setFormE]=useState({nombre:'',email:'',telefono:''});const[savingE,setSavingE]=useState(false)
async function guardarEdicion(){
if(!formE.nombre||!formE.email){showT('Nombre y correo requeridos','error');return}
setSavingE(true)
try{const{data:{session}}=await supabase.auth.getSession()
const resp=await fetch('https://xtqwbyfolbmiidiurmch.supabase.co/functions/v1/crear-delegado',{method:'POST',headers:{'Authorization':'Bearer '+(session?.access_token||''),'Content-Type':'application/json'},body:JSON.stringify({actualizar_delegado:true,id:editDel.id,nombre:formE.nombre.trim(),email:formE.email.trim().toLowerCase(),telefono:formE.telefono||''})})
const data=await resp.json()
if(data.error)throw new Error(data.error)
showT('Delegado actualizado'+(data.email_cambiado?'. Usa "Reenviar invitacion" para enviarle sus credenciales al correo corregido.':''))
setEditDel(null);refetch()
}catch(e){showT('Error: '+e.message,'error')}
setSavingE(false)}
async function reenviarInvitacion(d){
...
else{window.alert('El correo NO pudo enviarse ('+(data.correo_error||'error desconocido')+').\n\nComparte manualmente estas credenciales:\nCorreo: '+d.email+'\nContrasena temporal: '+data.password_temporal)}
}catch(e){showT('Error: '+e.message,'error')}
setReenviando(null)}
if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando...</div>
...
{delegados.map(d=>{const cd=cops.filter(c=>c.delegado_id===d.id);return(<div key={d.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:16,padding:18}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}><div style={{width:40,height:40,background:'#e8f4fd',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><span style={{fontSize:16,fontWeight:900,color:GB}}>{d.nombre?d.nombre[0].toUpperCase():'?'}</span></div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:900,color:GD,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.nombre}</div><div style={{fontSize:11,color:'#9ca3af',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{d.email}</div></div></div><div style={{fontSize:11,color:'#9ca3af',marginBottom:4}}>EDIFICIOS ({cd.length})</div>{cd.map(c=><div key={c.id} style={{fontSize:11,background:'#f0f7ff',color:GB,borderRadius:5,padding:'2px 7px',display:'inline-block',marginRight:4,marginBottom:3}}>{c.nombre}</div>)}<div style={{marginTop:10,display:'flex',justifyContent:'space-between',alignItems:'center',gap:6,flexWrap:'wrap'}}><Bdg t={d.activo?'Activo':'Inactivo'} color={d.activo?'#059669':'#dc2626'}/><div style={{display:'flex',gap:5}}><button onClick={()=>{setEditDel(d);setFormE({nombre:d.nombre||'',email:d.email||'',telefono:d.telefono||''})}} style={{background:'#f3f4f6',border:'none',borderRadius:7,padding:'4px 9px',cursor:'pointer',fontSize:11,fontWeight:700}}>Editar</button><button onClick={()=>reenviarInvitacion(d)} disabled={reenviando===d.id} style={{background:'#e8f4fd',border:'none',borderRadius:7,padding:'4px 9px',cursor:reenviando===d.id?'wait':'pointer',fontSize:11,fontWeight:700,color:GB}}>{reenviando===d.id?'Enviando...':'Reenviar invitacion'}</button><button onClick={async()=>{const{data:r,error}=await supabase.from('perfiles').update({activo:!d.activo}).eq('id',d.id).select();if(error){showT('Error: '+error.message,'error');return}if(!r||r.length===0){showT('No se pudo cambiar el estado (sin permisos)','error');return}showT(d.activo?'Delegado desactivado':'Delegado activado');refetch()}} style={{background:'#f3f4f6',border:'none',borderRadius:7,padding:'4px 9px',cursor:'pointer',fontSize:11,fontWeight:700}}>{d.activo?'Desactivar':'Activar'}</button></div></div></div>)})}
</div>
{editDel&&<Modal title={'Editar delegado'} onClose={()=>setEditDel(null)}>
<F label="Nombre" req><input style={inp} value={formE.nombre} onChange={e=>setFormE(p=>({...p,nombre:e.target.value}))}/></F>
<F label="Correo electronico" req><input style={inp} type="email" value={formE.email} onChange={e=>setFormE(p=>({...p,email:e.target.value}))} placeholder="correo@ejemplo.com"/></F>
<F label="Telefono"><input style={inp} value={formE.telefono} onChange={e=>setFormE(p=>({...p,telefono:e.target.value}))}/></F>
<div style={{background:'#fffbeb',borderRadius:10,padding:11,fontSize:12,color:'#92400e',marginBottom:12}}>Si corriges el correo, despues usa "Reenviar invitacion" para que le lleguen las credenciales a la direccion correcta.</div>
<button onClick={guardarEdicion} disabled={savingE} style={{width:'100%',background:savingE?'#9ca3af':GB,color:'#fff',border:'none',borderRadius:10,padding:11,fontWeight:800,cursor:savingE?'wait':'pointer'}}>{savingE?'Guardando...':'Guardar cambios'}</button>
</Modal>}
...
const menuItems=[...(perfil.rol==='director'?[{key:'panel',label:'Panel de Control',icon:TrendingUp}]:[]),{key:'copropiedades',label:'Copropiedades',icon:Building2},{key:'alertas',label:'Alertas',icon:Bell},...(perfil.rol==='director'?[{key:'delegados',label:'Delegados',icon:Users}]:[]),{key:'miperfil',label:'Mi Perfil',icon:UserCog}]
...
{selCop&&subTab==='replegal'&&<RepresentacionLegal copropiedad={selCop}/>}
