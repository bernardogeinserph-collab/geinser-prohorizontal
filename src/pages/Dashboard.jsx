...
import{Building2,Users,Briefcase,Shield,CheckSquare,Wrench,TrendingUp,LogOut,Plus,Edit2,Trash2,X,CheckCircle,AlertTriangle,ArrowLeft}from'lucide-react'
  ...
  return(
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:16}}>
        {cops.length===0&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:48,color:'#9ca3af'}}><Building2 size={40} color="#e5e7eb" style={{margin:'0 auto 12px'}}/><p>Sin copropiedades</p></div>}
        {cops.map(c=>{
...
function ModuloBase({tabla,copropiedad,titulo,color,campos,FD,renderCard}){
  ...
  async function save(){
    ...
    Object.keys(payload).forEach(k=>{if(!isNaN(payload[k])&&payload[k]!=='')payload[k]=Number(payload[k])})
...
function Contratos({copropiedad}){
  return<ModuloBase tabla="contratos" copropiedad={copropiedad} titulo="Contratos" color="#7c3aed"
    ...
    renderCard={(c,onEdit,onDel)=>{
      const d=daysTo(c.fecha_vencimiento)
      return(<div key={c.id} style={{background:'#fff',border:`1.5px solid ${d!==null&&d>=0&&d<=60?'#fca5a5':'#e5e7eb'}`,borderRadius:14,padding:16}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
          <span style={{background:'#f5f3ff',color:'#7c3aed',fontSize:11,borderRadius:20,padding:'2px 9px',fontWeight:700}}>{c.estado}</span>
          {d!==null&&d>=0&&d<=60&&<span style={{background:'#fef2f2',color:'#b91c1c',fontSize:10,borderRadius:20,padding:'2px 7px',fontWeight:700,border:'1px solid #fca5a5'}}>{d}d</span>}
        </div>
        <div style={{fontWeight:900,color:'#7c3aed'}}>{c.proveedor}</div>
        <div style={{fontSize:12,color:GD,fontWeight:700,marginBottom:6}}>{c.servicio}</div>
...
function Tareas({copropiedad}){
  ...
  return<ModuloBase tabla="tareas" copropiedad={copropiedad} titulo="Tareas" color={GB}
    ...
    renderCard={(t,onEdit,onDel)=>(
      <div key={t.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:14,padding:16}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
          <span style={{background:colP[t.prioridad]+'18',color:colP[t.prioridad],fontSize:10,borderRadius:20,padding:'2px 9px',fontWeight:700}}>{t.prioridad}</span>
          <span style={{background:t.estado==='Completada'?'#e6f9f0':'#f3f4f6',color:t.estado==='Completada'?'#0d7a4a':'#374151',fontSize:10,borderRadius:20,padding:'2px 8px',fontWeight:700}}>{t.estado}</span>
        </div>
...
function Mantenimiento({copropiedad}){
  return<ModuloBase tabla="mantenimientos" copropiedad={copropiedad} titulo="Mantenimiento" color="#059669"
    ...
    renderCard={(m,onEdit,onDel)=>{
      const d=daysTo(m.proxima_fecha)
      return(<div key={m.id} style={{background:'#fff',border:`1.5px solid ${d!==null&&d>=0&&d<=7?'#bbf7d0':'#e5e7eb'}`,borderRadius:14,padding:16}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
          <span style={{background:'#dcfce7',color:'#166534',fontSize:10,borderRadius:20,padding:'2px 9px',fontWeight:700}}>{m.estado}</span>
          {d!==null&&d>=0&&d<=7&&<span style={{background:'#fef9c3',color:'#854d0e',fontSize:10,borderRadius:20,padding:'2px 7px',fontWeight:700}}>{d}d</span>}
        </div>
        <div style={{fontWeight:900,color:GD}}>{m.area}</div>
        <div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>{m.proveedor||'Sin proveedor'} · {m.periodicidad}</div>
...
function Cartera({copropiedad}){
  return<ModuloBase tabla="cartera" copropiedad={copropiedad} titulo="Cartera" color="#dc2626"
    ...
    renderCard={(c,onEdit,onDel)=>(
      <div key={c.id} style={{background:'#fff',border:'1.5px solid #fecaca',borderRadius:14,padding:16}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
          <span style={{background:'#fef2f2',color:'#b91c1c',fontSize:10,borderRadius:20,padding:'2px 9px',fontWeight:700}}>{c.estado}</span>
          <span style={{fontSize:11,fontWeight:700,color:'#dc2626'}}>{fmtM(c.monto_deuda)}</span>
        </div>
        <div style={{fontWeight:900,color:GD}}>{c.concepto}</div>
        <div style={{fontSize:12,color:'#6b7280',marginBottom:4}}>{c.deudor||'—'} {c.unidad?'· '+c.unidad:''}</div>
...
function SGSST({copropiedad}){
  return<ModuloBase tabla="sgsst" copropiedad={copropiedad} titulo="SG-SST" color="#0891b2"
    ...
    renderCard={(a,onEdit,onDel)=>(
      <div key={a.id} style={{background:'#fff',border:'1.5px solid #e0f2fe',borderRadius:14,padding:16}}>
        <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
          <span style={{background:a.completada?'#e6f9f0':'#f0f9ff',color:a.completada?'#0d7a4a':'#0369a1',fontSize:10,borderRadius:20,padding:'2px 9px',fontWeight:700}}>{a.completada?'Ejecutada':'Pendiente'}</span>
        </div>
      ...
      <div style={{width:230,background:GD,height:'100vh',position:'fixed',left:0,top:0,zIndex:200,display:'flex',flexDirection:'column'}}>
        ...
        <nav style={{flex:1,padding:'6px 0',overflowY:'auto'}}>
          {!selectedCop&&[{key:'copropiedades',label:'Copropiedades',icon:Building2},...(perfil.rol==='director'?[{key:'delegados',label:'Delegados',icon:Users}]:[])].map(({key,label,icon:Icon})=>(
            <button key={key} onClick={()=>setTab(key)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 20px',border:'none',background:tab===key?'rgba(30,111,174,0.4)':'transparent',color:tab===key?'#fff':'rgba(255,255,255,0.55)',cursor:'pointer',fontSize:13,fontWeight:tab===key?700:500}}><Icon size={15}/>{label}</button>
          ))}
          {selectedCop&&subItems.map(({key,label,icon:Icon})=>(
            <button key={key} onClick={()=>setSubTab(key)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 20px 10px 24px',border:'none',background:subTab===key?'rgba(30,111,174,0.4)':'transparent',color:subTab===key?'#fff':'rgba(255,255,255,0.55)',cursor:'pointer',fontSize:12,fontWeight:subTab===key?700:500}}><Icon size={14}/>{label}</button>
          ))}
        </nav>
...
function DelegadosPanel(){
  const{data,loading}=useTable('perfiles')
  const delegados=data.filter(p=>p.rol==='delegado')
  if(loading)return<div style={{padding:40,textAlign:'center',color:'#9ca3af'}}>Cargando...</div>
  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        <h2 style={{margin:0,fontSize:20,fontWeight:900,color:GD}}>Delegados ({delegados.length})</h2>
        <div style={{background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:10,padding:'8px 14px',fontSize:12,color:'#1e40af'}}>Para crear delegados ve a Supabase → Authentication → Users</div>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:14}}>
        {delegados.length===0&&<div style={{gridColumn:'1/-1',textAlign:'center',padding:40,color:'#9ca3af'}}><Users size={36} color="#e5e7eb" style={{margin:'0 auto 10px'}}/><p>Sin delegados</p></div>}
        {delegados.map(d=>(
          <div key={d.id} style={{background:'#fff',border:'1.5px solid #e5e7eb',borderRadius:14,padding:18}}>
            <div style={{width:44,height:44,background:'#e8f4fd',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
              <span style={{fontSize:18,fontWeight:900,color:GB}}>{d.nombre?d.nombre[0].toUpperCase():'?'}</span>
            </div>
            <div style={{fontWeight:900,fontSize:15,color:GD}}>{d.nombre}</div>
            <div style={{fontSize:12,color:'#6b7280'}}>{d.email}</div>
            {d.telefono&&<div style={{fontSize:12,color:'#6b7280'}}>{d.telefono}</div>}
            <span style={{display:'inline-block',marginTop:6,background:d.activo?'#e6f9f0':'#fef2f2',color:d.activo?'#0d7a4a':'#b91c1c',fontSize:10,borderRadius:20,padding:'2px 8px',fontWeight:700}}>{d.activo?'Activo':'Inactivo'}</span>
          </div>
        ))}
      </div>
    </div>
